...
) ENGINE=InnoDB AUTO_INCREMENT=50 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='Действие';
...
INSERT INTO `crontabs` VALUES (1,NULL,NULL,'/api/action/toggle',1,'* * * * *','POST','[]','{\"id\":\"3\",\"table\":\"groups\"}',2,'2023-04-03 14:12:00','Включает/отключает группу модераторы каждую минуту',1),(2,NULL,NULL,'/api/action/clean-logs',1,'0 0 */1 * *','POST','[]','[]',2,'2023-04-04 00:00:00','Периодическая очистка логов',1),(3,NULL,NULL,'/api/action/erase-group',1,'0 0 * * *','POST','[]','{\"table\":\"groups\",\"id\":\"5\"}',2,'2023-04-04 00:00:00','Очистка от всех демо моделей раз в сутки',1),(4,NULL,NULL,'/api/action/anycomment',1,'0 */1 * * *','GET','[]','[]',2,'2023-04-03 15:00:00','Обновление инфы о новых комментариях и запросах в тех поддержку',1),(5,'{\"min\":33,\"max\":44,\"load\":40,\"fixed\":1}','sleep 3','',0,'* * * * *','CLI','null','[]',2,'2023-04-03 14:12:00','Перманентно спит по 3 сек в 33 потока одновременно, а если нагрузка более 40% даже не начинает спать',1);
...
CREATE TABLE `modals` (
  ...
  KEY `owner_group_2` (`owner_group`,`groups`(5)),
  FULLTEXT KEY `groups` (`groups`)
) ENGINE=InnoDB AUTO_INCREMENT=44 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='Модальное окно';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `settings` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=27 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='Настройки';
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2023-04-03 14:11:55
