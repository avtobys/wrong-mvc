 ...
 * 
 * Файл public_html/cron.php - запускает сам себя curl http запросом каждую минуту загружая метод load(), который в свою очередь посылает короткие независимые curl 
 * запросы на выполнение задач с таймаутом 0.001 сек и секретным токеном к тому же public_html/cron.php, в котором происходит выполение каждой задачи по отдельности.
 ...
 * После отправки коротких запросов вызывается Wrong\Task\Cron::set_run_at() который проставляет всем задачам следующее(очередное) их время выполнения. 
 * 
 * Таким образом реализуется бесконечный ежеминутный цикл, с выполнением задач которые подходят под расписание.
 * Запуск происходит лишь в одном потоке - если вызвать скрипт public_html/cron.php чаще чем раз в минуту,
