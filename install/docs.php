 ...
 * 
 * За переменные среды отвечает Env.php
 * 
 ...
 * автоматически скрываются средствами внедряющегося css - им добавляется правило display:none!important; За это отвечает Hideout.php метод hide
 ...
 * 
 * За запись логов отвечает класс Write.php
 */ 
 ...
 * 
 * Запросы к внутрисистемному(на текущем хосте) API выполняются при помощи статического метода req() в API.php
 * 
 * Если запрос выполняется от имени пользователя, в заголовках необходимо указать его X-Auth-Token, при этом работа через API должна быть включена на уровне системы,
 ...
 * 
 * При помощи статического метода req_external() из API.php
 ...
 * 
 * Вы можете использовать Hcaptcha.php на любых своих формах записывая неудачные попытки на единицу времени и вызывая капчу лишь после их
 ...
 * 
 * @note
 * На локальном сервере под ip 127.0.0.1 работу капчи вы не увидите, поскольку в методе check() класса Hcaptcha.php
 ...
 * - сбор информации;
 * 
 * За техническую реализацию встроенного cron отвечает файл cron.php и класс Сron.php
 * 
