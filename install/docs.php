 ...
 * 
 * Установка дополнительных переменных среды происходит в файле session.php
 * 
 * За переменные среды отвечает env.class.php
 * 
 ...
 * - Cron задачи
 * 
 * У моделей типа <b>Страницы, Выборки, Модальные окна, Действия</b> помимо группы владельца, есть группы доступа - это группы которым такая модель доступна по http через контроллер uri-router.php
 ...
 * 
 * Например действие удаление - rm.php(модель /api/action/rm) наша группа пользователя должна быть включена в группы доступа у модели rm,
 ...
 * автоматически скрываются средствами внедряющегося css - им добавляется правило display:none!important; За это отвечает hideout.class.php метод hide
 * вызывающийся в session.php по завершении работы скрипта.
 * 
 * @note
 ...
 * 
 * 
 * Для моделей с динамическими uri используйте всего одну запись в таблице моделей и один request и дописывайте необходимую php логику в uri-router.php
 ...
 * 
 * Для моделей с динамическими uri используйте одну запись в таблице моделей и один request и дописывайте необходимую php логику в uri-router.php
 * 
 ...
 * 
 * За запись логов отвечает write.class.php
 */ 
 ...
 * 
 * Запросы к внутрисистемному(на текущем хосте) API выполняются при помощи статического метода req() в api.class.php
 * 
 * Если запрос выполняется от имени пользователя, в заголовках необходимо указать его X-Auth-Token, при этом работа через API должна быть включена на уровне системы,
 ...
 * 
 * При помощи статического метода req_external() из api.class.php
 ...
 * Токен представляет из себя md5 хеш идентификатора сессии, задаётся в session.php - там автоматически добавляется в страницу javascript переменная с токеном,
 ...
 * 
 * Вы можете использовать hcaptcha.class.php на любых своих формах записывая неудачные попытки на единицу времени и вызывая капчу лишь после их
 ...
 * @endcode
 * 
 * Поймав такой ответ, фронтенд вызывает модальное окно с капчей hcaptcha.php поверх окна авторизации
 * @code
 ...
 * 
 * @note
 * На локальном сервере под ip 127.0.0.1 работу капчи вы не увидите, поскольку в методе check() класса hcaptcha.class.php
 ...
 * - сбор информации;
 * 
 * За техническую реализацию встроенного cron отвечает файл cron.php и cron.class.php
 * 
 * Файл cron.php - запускает сам себя curl http запросом каждую минуту загружая класс, в котором уже происходит выполнение задач, которые должны быть выполнены исходя из их логики заданных раписаний.
 * Таким образом реализуется бесконечный ежеминутный цикл. Запуск происходит лишь в одном потоке - если вызвать скрипт cron.php чаще чем раз в минуту,
 ...
 * которая инжектится в session.php при помощи <a target="_blank" href="/docs/stackjs.html"><b>js стека</b></a>(не всегда, а только если не активирован цикл автозапусков).
 * Это не совсем надежно, если на вашем сайте мало посетителей, т.к. иногда http запрос может не выполниться и цикл самозапуска cron.php прервется.
 * Бывает весьма редко, и если на сайте совсем никто не бывает, в основном цикл работает как часы.
 * Но чтобы повысить надежность, вы можете дополнительно поставить выполнение cron.php на свой системный cron раз минуту, вреда от этого не будет
