...
/**
 * @file
 * @brief окно добавления нового модального окна
 */
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fa fa-window-restore mr-2"></i>Добавить модальное окно</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body pt-2">
                <form action="<?= Wrong\Models\Actions::find(7)->request ?>">
                    ...
                    <div class="input-group input-group-sm mt-2">
                        ...
                        <select name="template_id" class="custom-select">
                            <?php
                            foreach (Wrong\Models\Templates::all_available('modal', 'type') as $row) {
                    ...
                    <div class="input-group input-group-sm mt-2">
                        ...
                        <input type="text" name="request" class="form-control" value="/api/modal/request" placeholder="/api/modal/request" autocomplete="off" required>
                    </div>
                    <div class="input-group input-group-sm mt-2">
                        <div class="input-group-prepend w-50">
                            <span class="input-group-text w-100">Файл обработчик</span>
                        </div>
                        <input title="Доступные каталоги: <?= implode(', ', $user->writeble_paths) ?>" type="text" name="file" class="form-control" value="/api/modal/<?= Wrong\Rights\Group::row($user->main_group_id)->path ?>/request.php" placeholder="/api/modal/<?= Wrong\Rights\Group::row($user->main_group_id)->path ?>/request.php" autocomplete="off" required>
                    </div>
                    <div class="bg-light-info border mt-2 px-2 py-1 rounded">
                        <div class="custom-control custom-checkbox small">
                            <input type="checkbox" name="add-action" class="custom-control-input" id="add-action">
                            <label class="custom-control-label" for="add-action">Добавить действие<i class="fa fa-flash ml-2"></i></label>
                        </div>
                    </div>
