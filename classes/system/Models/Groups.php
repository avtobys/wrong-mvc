...
class Groups extends Controller implements ModelsInterface
{
    ...
    public static function create($arr)
    {
        ...
        $sth = $dbh->prepare("INSERT INTO `groups` (`name`, `owner_group`, `weight`, `path`) VALUES (:name, :owner_group, :weight, :path)");
                ...
                foreach (['actions', 'modals', 'selects', 'pages', 'users', 'templates'] as $table) {
                    foreach ($dbh->query("SELECT * FROM `$table`") as $row) {
                        ...
                        if (Group::is_users($array) || Group::is_not_system($array)) {
                            ...
                            $dbh->query("UPDATE `$table` SET `groups` = '" . json_encode($array) . "' WHERE `id` = $row->id");
                        }
                    }
                }
            }
