...
class Cache
{
     ...
     * @param int $timeout - если указано, то сверка будет происходить в соответствии с указанным таймаутом(на случай если таймаут был изменен уже после кеширования)
     * 
     * @return string Если кеша таймаут не истек, то возвращает записанные данные. Иначе ничего не возвращается.
     */
    public function get($key, $timeout = 0)
    {
        $filename = $this->getFile($key);
        if (file_exists($filename) && filemtime($filename) >= time()) {
            if ($timeout && $timeout < filemtime($filename) - time()) return; // указанный таймаут вышел
